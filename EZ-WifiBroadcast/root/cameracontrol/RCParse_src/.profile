...
###### For test without camera: ###############echo "nice -n -9 cat /dev/zero  - | nice -n -9 /root/wifibroadcast/tx_rawsock -p 0 -b $VIDEO_BLOCKS -r $VIDEO_FECS -f $VIDEO_BLOCKLENGTH -t $VIDEO_FRAMETYPE -d $VIDEO_WIFI_BITRATE -y 0 $NICS" >> /dev/shm/startReadCameraTransfer.sh

chmod +x /dev/shm/startReadCameraTransfer.sh
#ifconfig eth0 192.168.1.49

/usr/bin/python /root/cameracontrol/cameracontrolUDP.py -IsCamera1Enabled $IsCamera1Enabled -IsCamera2Enabled $IsCamera2Enabled -IsCamera3Enabled $IsCamera3Enabled -IsCamera4Enabled $IsCamera4Enabled  -Camera1ValueMin $Camera1ValueMin -Camera1ValueMax $Camera1ValueMax -Camera2ValueMin $Camera2ValueMin -Camera2ValueMax $Camera2ValueMax -Camera3ValueMin $Camera3ValueMin -Camera3ValueMax $Camera3ValueMax  -Camera4ValueMin $Camera4ValueMin -Camera4ValueMax $Camera4ValueMax -DefaultCameraId $DefaultCameraId

#/usr/bin/python /root/cameracontrol/cameracontrol.py
...
#sleep 1
#/root/my/startVideoCapture &
#/root/my/startVideoTransfer.sh
#MYADDEND
...
function osdtx_function {
	...
	nice cat $FC_TELEMETRY_SERIALPORT | nice /root/cameracontrol/RCParseCh $ChannelToListen | nice /root/wifibroadcast/tx_telemetry -p 1 -c $TELEMETRY_CTS -r 2 -x $TELEMETRY_TYPE -d 12 -y 0 $NICS
...
function hotspot_check_function {
	...
	if [ "$ETHERNET_HOTSPOT" == "Y" ]; then
	    ...
	    nice u
dhcpd -I 192.168.1.1 /etc/udhcpd-eth.conf
	fi
